...
function showMealList() {
        ...
        meals.then((data) => {
            ...
            if (data.meals) {
                data.meals.forEach((element) => {
                    let isFav=false;
                for (let index = 0; index < arr.length; index++) {//checks if the current meal (element) is favorited by comparing its idMeal with the items in the arr array, which contains IDs of favorited meals from local storage.
    ...
    }, 2000); // Delay of 2000ms (2 seconds) before making the API call
}
